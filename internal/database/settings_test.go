...
func TestSettings_ListAll(t *testing.T) {
	...
	user1, err := Users(tx).Create(ctx, NewUser{Username: "u1"})
	if err != nil {
		t.Fatal(err)
	}
	user2, err := Users(tx).Create(ctx, NewUser{Username: "u2"})
	if err != nil {
		t.Fatal(err)
	}

	// Try creating both with non-nil author and nil author.
	if _, err := Settings(tx).CreateIfUpToDate(ctx, api.SettingsSubject{User: &user1.ID}, nil, &user1.ID, `{"abc": 1}`); err != nil {
		t.Fatal(err)
	}
	if _, err := Settings(tx).CreateIfUpToDate(ctx, api.SettingsSubject{User: &user2.ID}, nil, nil, `{"xyz": 2}`); err != nil {
		t.Fatal(err)
	}

	t.Run("all", func(t *testing.T) {
		settings, err := Settings(tx).ListAll(ctx, "")
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("impreciseSubstring", func(t *testing.T) {
		settings, err := Settings(tx).ListAll(ctx, "xyz")
		if err != nil {
			t.Fatal(err)
		}
...
func TestCreateIfUpToDate(t *testing.T) {
	...
	tx := dbtest.NewFastTx(t)
	ctx := context.Background()
	u, err := Users(tx).Create(ctx, NewUser{Username: "test"})
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("quicklink with safe link", func(t *testing.T) {
		...
		_, err := Settings(tx).CreateIfUpToDate(ctx, api.SettingsSubject{User: &u.ID}, nil, nil, contents)
		if err != nil {
			t.Fatal(err)
		}
	})
...
func TestGetLatestSchemaSettings(t *testing.T) {
	...
	if _, err := Settings(tx).CreateIfUpToDate(ctx, api.SettingsSubject{User: &user1.ID}, nil, &user1.ID, `{"search.uppercase": true }`); err != nil {
		t.Error(err)
	}

	settings, err := Settings(tx).GetLastestSchemaSettings(ctx, api.SettingsSubject{User: &user1.ID})
	if err != nil {
		t.Fatal(err)
	}
