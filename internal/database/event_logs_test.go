...
func TestEventLogs_CountUniqueUsersPerPeriod(t *testing.T) {
	...
	for _, e := range events {
		if err := EventLogs(tx).Insert(ctx, e); err != nil {
			t.Fatal(err)
		}
	}

	values, err := EventLogs(tx).CountUniqueUsersPerPeriod(ctx, Daily, now, 3, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEventLogs_UsersUsageCounts(t *testing.T) {
	...
	have, err := EventLogs(tx).UsersUsageCounts(ctx)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEventLogs_SiteUsage(t *testing.T) {
	...
	el := &eventLogStore{Store: basestore.NewWithDB(tx, sql.TxOptions{})}
...
func TestEventLogs_TestCodeIntelligenceRepositoryCounts(t *testing.T) {
	...
	for _, repo := range repos {
		...
		if _, err := tx.Exec(query.Query(sqlf.PostgresBindVar), query.Args()...); err != nil {
	...
	t.Run("All", func(t *testing.T) {
		counts, err := EventLogs(tx).CodeIntelligenceRepositoryCounts(ctx)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("ByLanguage", func(t *testing.T) {
		counts, err := EventLogs(tx).CodeIntelligenceRepositoryCountsByLanguage(ctx)
		if err != nil {
			t.Fatal(err)
		}
	...
	for _, name := range names {
		for _, day := range days {
			for i := 0; i < 25; i++ {
				...
				g.Go(func() error {
					return EventLogs(tx).Insert(gctx, e)
				})
			}
		}
	}
...
func TestEventLogs_AggregatedSparseSearchEvents(t *testing.T) {
	...
	events, err := EventLogs(tx).AggregatedSearchEvents(ctx, now)
	if err != nil {
		t.Fatal(err)
	}
		...
		Argument: json.RawMessage(`
{
   "code_search":{
	...
	if err := EventLogs(tx).Insert(gctx, e); err != nil {
		t.Fatal(err)
	}
...
func TestEventLogs_ListAll(t *testing.T) {
	...
	for _, event := range events {
		if err := EventLogs(tx).Insert(ctx, event); err != nil {
			t.Fatal(err)
		}
	}

	searchResultQueriedEvent := "SearchResultsQueried"
	have, err := EventLogs(tx).ListAll(ctx, EventLogsListOptions{EventName: &searchResultQueriedEvent})
	if err != nil {
		t.Fatal(err)
	}
...
func TestEventLogs_LatestPing(t *testing.T) {
	...
	t.Run("with no pings in database", func(t *testing.T) {
		ctx := context.Background()
		ping, err := EventLogs(tx).LatestPing(ctx)
		if ping != nil {
	...
	t.Run("with existing pings in database", func(t *testing.T) {
		...
		gotPing, err := EventLogs(tx).LatestPing(ctx)
		if err != nil || gotPing == nil {
			t.Fatal(err)
		}
		expectedPing := &types.Event{
			ID:              gotPing.ID, // ignore different ID
			Name:            events[1].Name,
			URL:             events[1].URL,
			UserID:          &userID,
